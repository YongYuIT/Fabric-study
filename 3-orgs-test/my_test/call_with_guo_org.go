...
func Call_with_guo() {
	//读取配置文件，创建SDK
	configProvider := config.FromFile("/mnt/hgfs/3-orgs-test/my_test/config-guo.yaml")
	sdk, err := fabsdk.New(configProvider)
	if err != nil {
		log.Fatalf("create sdk fail: %s\n", err.Error())
	}

	//读取配置文件(config.yaml)中的组织(guo.thinking.com)的用户(Admin)
	mspClient, err := mspclient.New(sdk.Context(),
		mspclient.WithOrg("guo.thinking.com"))
	if err != nil {
	...
	channelProvider := sdk.ChannelContext("mythnkingchannel",
		...
		fabsdk.WithOrg("guo.thinking.com"))
